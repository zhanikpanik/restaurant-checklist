...
export async function POST({ request }) {
  ...
  try {
    ...
    console.log(
      `📝 Creating custom product: ${name}, section_id: ${finalSectionId}, tenant: ${tenantId}`,
    );
    ...
    console.log(
      `✅ Created product: ${result.rows[0].name} (ID: ${result.rows[0].id})`,
    );
...
export async function PUT({ request }) {
  const tenantId = getTenantId(request);
  const { id, name, unit, categoryId, minQuantity, currentQuantity } =
    await request.json();
  const { client, error } = await getDbClient();
