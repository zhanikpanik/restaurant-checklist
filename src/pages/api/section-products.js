...
export async function GET({ request }) {
  ...
  try {
    // Get optional section_id filter from query params
    ...
    if (sectionId) {
      ...
      query = `
                ...
                WHERE s.restaurant_id = $1 AND sp.section_id = $2 AND sp.is_active = true
                ORDER BY sp.name ASC;
            `;
      params = [tenantId, sectionId];
    } else {
      // Get all section products for this tenant
      query = `
                SELECT
                    sp.id,
                    sp.name,
