import { getTenantId, getTenantConfig } from '../../lib/tenant-manager.js';
...
export async function POST({ request }) {
    try {
        ...
        const { orderId, supplier_id, storage_id, supply_date, products } = await request.json();
        ...
        for (const product of products) {
            if (!product.product_id) {
                throw new Error('Each product must have a product_id (Poster product ID)');
            }
        ...
        const posterApiUrl = 'https://joinposter.com/api/incomingOrders.createIncomingOrder';
        ...
        products.forEach((product, index) => {
            ...
            supplyData[`products[${index}][incoming_quantity]`] = product.incoming_quantity;
        ...
        console.log('📥 Poster API response:', JSON.stringify(responseData, null, 2));

        // Check for Poster API errors
        if (responseData.error) {
            throw new Error(`Poster API Error: ${responseData.error} - ${responseData.message || 'Unknown error'}`);
        }
        ...
        console.log(`✅ Supply created successfully in Poster! Supply ID: ${responseData.response?.incoming_order_id || 'N/A'}`);
