...
export async function GET(request: NextRequest) {
  try {
    ...
    const orders = await withTenant(restaurantId, async (client) => {
      ...
      // 4. DATA ENRICHMENT: For pending/sent orders, backfill supplier/cost for items
      // that were created before the self-learning logic was active (POST /orders saves
      // supplier_id on creation now, so enrichment is a fallback for legacy orders).
      const activeOrders = orders.filter(o => o.status === 'pending' || o.status === 'sent');
      
      if (activeOrders.length > 0) {
        // Collect product IDs that need enrichment (missing supplier or price)
        const productIdsToEnrich = new Set<number>();
        activeOrders.forEach(order => {
          (order.order_data?.items || []).forEach((item: any) => {
            if (item.productId && (!item.supplier || !item.price)) {
              productIdsToEnrich.add(item.productId);
            }
          });
        });

        if (productIdsToEnrich.size > 0) {
          const ids = Array.from(productIdsToEnrich);
          const enrichmentResult = await client.query(`
            ...
            WHERE sp.id = ANY($1) AND sec.restaurant_id = $2 AND sp.is_active = true
          `, [ids, restaurantId]);
