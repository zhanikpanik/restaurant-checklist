          ...
          <div className="flex gap-1 bg-slate-700/50 p-1 rounded-xl">
            <button
              ...
              className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-colors ${
                activeTab === "active" ? "bg-white text-slate-900" : "text-slate-400 hover:text-white"
              }`}
            >
              Активные
              {(hasInTransit || hasPending) && (
                <span className={`ml-2 w-2 h-2 rounded-full inline-block ${
                  hasInTransit ? "bg-green-400" : "bg-amber-400"
                }`} />
              )}
            </button>
            <button
          ...
          <>
            ...
            {!hasInTransit && !hasPending && (
              <div className="text-center py-16">
                ...
                <p className="text-slate-400 mb-6">Нет активных заказов</p>
                <Link
                  href="/"
